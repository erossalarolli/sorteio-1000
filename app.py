        ...
        conn.execute("""
            ...
            BEGIN
                IF NOT EXISTS (
                    ...
                    ALTER TABLE reservas ADD CONSTRAINT reservas_cpf_key UNIQUE (cpf);
                END IF;
            END$$;
        """)
        conn.commit()
    ...
    try:
        with db() as conn:
            ...
            if row:
                ...
                # Descobre se o CPF, telefone ou número já foram usados.
                existente = conn.execute("""
                    SELECT numero, cpf, telefone FROM reservas
                    WHERE cpf = %s OR telefone = %s OR numero = %s
                    LIMIT 1
                """, (cpf, telefone, numero)).fetchone()
                conn.rollback()

                if existente and existente["numero"] != numero:
                    if existente["cpf"] == cpf:
                        flash(f"Este CPF já reservou o número {existente['numero']:04d}.")
                    elif existente["telefone"] == telefone:
...
def admin_editar_reserva(reserva_id):
    ...
    try:
        ...
        return jsonify({"erro": "Esse número, WhatsApp ou CPF já está em uso por outra reserva."}), 409
    except Exception:
