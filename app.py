...
def init_db():
    ...
    # Reforço: remove QUALQUER constraint única (por qualquer nome) que ainda
    # exista isoladamente sobre a coluna telefone. Isso cobre o caso de a
    # constraint ter sido criada com outro nome (ex: em bancos antigos/migrados),
    # o que fazia o "ON CONFLICT DO NOTHING" genérico do /reservar barrar o
    # cadastro por telefone repetido e mostrar, por engano, a mensagem de
    # "número já reservado".
    with db() as conn:
        conn.execute("""
            DO $$
            ...
            BEGIN
                ...
                FROM pg_attribute
                WHERE attrelid = 'reservas'::regclass AND attname = 'telefone';
    ...
    # O mesmo reforço, agora para a coluna cpf.
    with db() as conn:
        conn.execute("""
            DO $$
            DECLARE
            ...
            BEGIN
                SELECT attnum INTO col_attnum
                FROM pg_attribute
                WHERE attrelid = 'reservas'::regclass AND attname = 'cpf';
    ...
    # Reforço extra: remove também qualquer ÍNDICE único isolado nessas colunas
    # que não esteja necessariamente amarrado a uma constraint nomeada.
    with db() as conn:
        conn.execute("""
            DO $$
            DECLARE
            ...
            BEGIN
                FOR r IN
                    ...
                    WHERE tablename = 'reservas'
                      ...
                      AND (indexdef ILIKE '%(telefone)%' OR indexdef ILIKE '%(cpf)%')
                LOOP
    ...
    # O número (numero) continua ÚNICO: cada número só pode pertencer a uma
    # única reserva/pessoa por vez. Isso é o que garante a integridade do sorteio.
    ...
    try:
        with db() as conn:
            ...
            if row:
                ...
                # (a mesma pessoa pode reservar vários números). Como o ON CONFLICT
                # agora está amarrado especificamente à coluna numero, chegar aqui
                # só é possível se o número em si já estiver ocupado.
                conn.rollback()
                flash("Esse número já foi reservado. Escolha outro.")

    except UniqueViolation:
        # Isso só deve acontecer se restar algum índice/constraint único
        # inesperado no banco (fora do numero). Não confundir com número
        # já reservado, cuja mensagem é tratada acima.
        flash("Não foi possível concluir a reserva: já existe um cadastro conflitante no banco. Contate o administrador.")
    except Exception:
...
def admin_diagnostico():
    ...
    with db() as conn:
        constraints = conn.execute("""
            SELECT conname AS nome, contype AS tipo,
        ...
        indices = conn.execute("""
            ...
            WHERE tablename = 'reservas'
        """).fetchall()
    return jsonify({"constraints": constraints, "indices": indices})
